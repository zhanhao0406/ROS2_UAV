...
```
然后开始下载PX4-Autopilot  
保持这个版本即可，不要升级；QGC也不能使用最新的，要降级，在后面有叙述。  
以下 git clone 部分出问题，参考[网络问题](#网络问题)     
以下 bash ~/PX4 部分出问题，参考[系统包版本问题](#系统包版本问题)   
以下 make px4_sitl 部分出问题，参考[make出现问题](#make出现问题)
...
### make出现问题
如果在make时出现报错：
1. 报错`/home/hzh/ws_xtd2_pyenv/bin/python3: Error while finding module specification for 'symforce.symbolic' (ModuleNotFoundError: No module named 'symforce')`， 是因为python虚拟环境中缺少symforce 的关键模块，可能是之前装的时候缺失了。重新安装即可`pip install symforce`;验证安装完成的方法`python -c "import symforce; print(symforce.__version__)"`,报出版本号即可。
2. 另一个报错`  Compatibility with CMake < 3.5 will be removed from a future version of CMake.
  Update the VERSION argument <min> value or use`。 这个不是报错，只是cmake版本过老的警告，不影响编译。但后续需要梯子进行github的编译，所以网络问题需要注意。
3. 还是不能成功，一直有子模块网络报错的原因。   
问题解决了，自己开热点即可正常编译。但编译到后期，会报错缺少`libmicroxrceddsclient`之类的部分。这部分是网络原因，如果无法解决，就先安装[下一部分](#配置MicroXRCEDDS)之后，回来再接着继续安装。
4. 直到安装完成前，都是需要有github的连接的。  

---
## 配置MicroXRCEDDS
通过Micro XRCE-DDS的帮助，ROS2可以和px4中uORB消息直接联系起来。  
以下 git clone 部分出问题，参考[安装DDS时网络问题](#安装DDS时网络问题)    
...
### 安装DDS时网络问题
注意：在git clone下，网络等多种都可能影响下载失败。  
解决办法同上，就是找到github上的镜像加速网站，使用这个链接进行git。 参考： https://gh-proxy.com/    
但是同样有问题，这个DDS可以使用加速下载，但是里面的make之后，也有github上的clone，无法通过这个加速链接git。    
make部分已经解决，单纯是网络原因。网络没问题就能编译成功。  

之后配置在PX4地方，没有make成功的部分，[继续第三点的安装](#make出现问题)。  
之后在PX4文件夹下，进入虚拟环境，直接`make px4_sitl`，然后 `make px4_sitl gz_x500`也都测试成功。  

---
## 下载QGroundControl地面站
<mark> 注意：PX4为v1.15.3，QGC不能用5.x.x，我下载的是4.2.8 !!! </mark> 
参考[官方教程链接](https://docs.qgroundcontrol.com/master/en/qgc-user-guide/getting_started/download_and_install.html)
```bash
sudo usermod -aG dialout "$(id -un)" # 启用串口访问。将您的用户添加到拨号组，以便您无需root权限即可与USB设备通信
sudo systemctl mask --now ModemManager.service # 禁用 ModemManager。在某些基于 Ubuntu 的系统中，ModemManager 可能会占用 QGC 所需的串口。

sudo apt install gstreamer1.0-plugins-bad gstreamer1.0-libav gstreamer1.0-gl -y
sudo apt install libfuse2 -y
sudo apt install libxcb-xinerama0 libxkbcommon-x11-0 libxcb-cursor-dev -y
```
然后下载QGC的appimage。  v 4.2.8 !!   
```bash
cd ~/Downloads/
sudo chmod +x QGroundControl.AppImage  # 使 AppImage 可执行
./QGroundControl.AppImage  # 运行 QGroundControl 可以在文件管理器中双击 AppImage 文件，也可以从终端启动它：
```

---
## 安装XTDrone2
安装的目的是什么，需要问问deepseek。这个模块的目的和作用。以及这个的配置部分，接口手则都没有完成编译。
